...
from model_wrapper import InnerStatesUsingWrapper
...
class ModelInside():
    ...
    def __init__(self, path_to_save_results, data_path_without_hallucinations, data_path_with_hallucinations,
                 model_name="meta-llama/Meta-Llama-3.1-8B", dataset_size=1000, dataset_name="triviaqa", threshold_of_data=None,
                 concat_answer=False,static_dataset=False, alice=False, persona=False, truthfulness=False, fake_alignment = False, realistic=False):
        print(f"{persona=} {alice=} {truthfulness=} {concat_answer=}  {realistic=}")
        ...
        if persona or (self.data_path_with_hallucinations!= None and "persona" in self.data_path_with_hallucinations.lower()):
            if not (alice or (
                    self.data_path_with_hallucinations != None and "alice" in self.data_path_with_hallucinations.lower())):
                self.path_to_save_results = self.path_to_save_results + "persona/"
        if truthfulness or (self.data_path_with_hallucinations!= None and "truthfulness" in self.data_path_with_hallucinations.lower()):
            if not (alice or (
                    self.data_path_with_hallucinations != None and "alice" in self.data_path_with_hallucinations.lower())):
                self.path_to_save_results = self.path_to_save_results + "truthfulness/"
        if realistic or (self.data_path_with_hallucinations!= None and "realistic" in self.data_path_with_hallucinations.lower()):
            if not (alice or (
                    self.data_path_with_hallucinations != None and "alice" in self.data_path_with_hallucinations.lower())):
                self.path_to_save_results = self.path_to_save_results + "realistic/"
    ...
    def generate_data(self):
        ...
        :return:
        """
        generate_text_func =  self.get_inner_model_info_no_context_dataset
    ...
    def generate_data_type_1(self,data_path, indeces_to_remove = []):


        type_1_data = self.load_dataset(data_path)
        if len(indeces_to_remove) > 0:
            type_1_data = [d for i, d in enumerate(type_1_data) if i not in indeces_to_remove]
        else:
    ...
    def get_type_1_data(self,path, indeces_to_remove = []):
        if os.path.exists(f"{self.path_to_save_results}_all_mlp_vector_type1.npy"):
            ...
            path,indeces_to_remove=indeces_to_remove)
        return all_mlp_vector_type1, all_attention_vector_type1, heads_vectors_type1, all_residual_vectors_type1
    ...
    def load_data(self, names):
        ...
        for name in names:
            data = None
            if os.path.exists(f"{self.path_to_save_results}_{name}.npy"):
    ...
    def get_inner_model_info_no_context_dataset(self, dataset, tag, concatenate_answer=False,static=False):
        ...
        if number_of_examples_used < self.dataset_size:
            self.dataset_size = number_of_examples_used
